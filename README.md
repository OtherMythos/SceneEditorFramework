...
The gizmos use `arrow.mesh` and `scaleHandle.mesh` from this repository's `res` directory, so add
that directory to the project's `OgreResources` configuration as well:

```json
"OgreResources": {
    "general": [
        ["FileSystem", "sceneEditorFramework/res"]
    ]
}
```
